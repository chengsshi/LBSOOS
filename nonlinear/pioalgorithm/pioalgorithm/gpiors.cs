    ...
    public class GPIOrs
    {
        public static void Process(Function type)
        {
            ...
            for (int run = 0; run < Parameter.Run; ++run)
            {
                ...
                trajectoryName = type.ToString() + ".gpiors." + Parameter.Dimension.ToString()
                                    + "." + run.ToString() + ".txt";
                Write.writeTrajectory(trajectoryName, ref trajectory);

                logSentence = type.ToString() + " fitness[gpiors] " + gbestValue.ToString()
                    + ", run " + run.ToString();
            ...
            logName = "_" + type.ToString() + ".gpiors." + Parameter.Dimension.ToString() + ".txt";

            // global count
            logSentence = type.ToString() + " gpiors count2 " + count2.ToString();
            Console.WriteLine(logSentence);
            log.Add(logSentence);

            logSentence = type.ToString() + " gpiors count4 " + count4.ToString();
            Console.WriteLine(logSentence);
            log.Add(logSentence);
            #region sort result
            fitnessRun.Sort();
