    ...
    public class GPIOr
    {
        public static void Process(Function type)
        {
            ...
            for (int run = 0; run < Parameter.Run; ++run)
            {
                ...
                trajectoryName = type.ToString() + ".gpior." + Parameter.Dimension.ToString()
                                    + "." + run.ToString() + ".txt";
                Write.writeTrajectory(trajectoryName, ref trajectory);

                logSentence = type.ToString() + " fitness[gpior] " + gbestValue.ToString()
                    + ", run " + run.ToString();
            ...
            logName = "_" + type.ToString() + ".gpior." + Parameter.Dimension.ToString() + ".txt";

            // global count
            logSentence = type.ToString() + " gpior count2 " + count2.ToString();
            Console.WriteLine(logSentence);
            log.Add(logSentence);

            logSentence = type.ToString() + " gpior count4 " + count4.ToString();
            Console.WriteLine(logSentence);
            log.Add(logSentence);
