            ...
            for (int run = 0; run < Parameter.Run; ++run)
            {
                ...
                for (int iter = 0; iter < Parameter.Iteration; ++iter)
                {
                    for (int atom = 0; atom < Parameter.Population; ++atom)
                    {
                        for (int dim = 0; dim < Parameter.Dimension; ++dim)
                        {
                            solution[dim] = population[atom, dim];
                        }
                        fitnessPopu[atom] = function(type, ref solution);

                        #region update personal best and global best
                        if (fitnessPopu[atom] < fitness[atom])
                        {
                            fitness[atom] = fitnessPopu[atom];
                        }

                        // update global best
                        if (fitness[atom] < fitness[global]) global = atom;
                        #endregion
                    }
                    ...
                    for (int atom = 0; atom < Parameter.Population; ++atom)
                    {
                        ...
                        int temIdx = acculateNumCluster[cluster[atom]]
                            + counterCluster[cluster[atom]] - 1; // -1 变为 序号
                        for (int dim = 0; dim < Parameter.Dimension; ++dim)
                        {
                    ...
                    for (int atom = 0; atom < Parameter.Population; ++atom)
                    {
                        ...
                        for (int dim = 0; dim < Parameter.Dimension; ++dim)
                        {
                            stepSize[dim] = Probability.logsig((0.5 * Parameter.Iteration
                                - iter) / 20.0) * rand.NextDouble();
                            //Console.WriteLine(tempSolution[dim]);
                            tempSolution[dim] += stepSize[dim]
                                * Probability.gaussian(rand.NextDouble()
                                * (iter / Parameter.size + 5))
                                * (rand.NextDouble() - 0.5);
                            // Console.WriteLine(tempSolution[dim]);
                    ...
                    trajectory[iter] = fitness[global];
                }
