        ...
        public static void BSOobjective(Function type)
        {
            ...
            for (int run = 0; run < Parameter.Run; ++run)
            {
                ...
                for (int ite = 0; ite < Parameter.Iteration; ++ite) //while current_iteration_number <= max_iteration_comp 
                {
                    ...
                    if ((ite + 1) % 100 == 0)
                    {
                        Archive.ArchiveUpdate(ref population, ref fitness, ref enough);
                    }
                ...
                Archive.AchiveMeasure(type, ref count2, ref count4, ref enough);
                #endregion
                ...
                logSentence = type.ToString() + " fitness [bsoos] " + globalValue.ToString() + " run " + run.ToString();
                Console.WriteLine(logSentence);
                log.Add(logSentence);
