...
namespace BSOBSOOS
{
    class LearningBSO
    {
        ...
        public static void process(Function type)
        {
            ...
            double[] clusterFitnessValues = new double[Parameter.cluster];
            ...
            for (int run = 0; run < Parameter.Run; ++run)
            {
                ...
                for (int ite = 0; ite < Parameter.Iteration; ++ite)
                {
                    ...
                    for (int index = 0; index < Parameter.cluster; ++index)
                    {
                        ...
                        clusterFitnessValues[index] = System.Double.MaxValue;
                    ...
                    for (int atom = 0; atom < Parameter.Population; ++atom)
                    {
                        ...
                        // minimization
                        if (clusterFitnessValues[cluster[atom]] > fitnessPopu[atom])
                        {
                            clusterFitnessValues[cluster[atom]] = fitnessPopu[atom];
                    ...
                    if (rand.NextDouble() < 0.2)
                    {
                        ...
                        for (int dim = 0; dim < Parameter.Dimension; ++dim)
                        {
                            centers[index, dim] = Parameter.Lower + (Parameter.Upper - Parameter.Lower) * rand.NextDouble();
                        }
                    }
                    ...
                    {
                        // probability for select one cluster to form new individual
                        double r1cluster = rand.NextDouble();
                        // select one cluster
                        if (r1cluster < oneCluster)
                        {
                        ...
                        else
                        {
                            ...
                            if (rand.NextDouble() < 0.5)
                            {
                                for (int dim = 0; dim < Parameter.Dimension; ++dim)
                                {
                                    tempSolution[dim] = rnum1 * centers[cluster_1, dim]
                                        + (1 - rnum1) * centers[cluster_2, dim];
                                }
                            }
                            ...
                            else
                            {
                                for (int dim = 0; dim < Parameter.Dimension; ++dim)
                                {
                                    tempSolution[dim] = rnum1 * popuSorted[indi_1, dim] + (1 - rnum1) * popuSorted[indi_2, dim];
                                }
                            }
                        }
                        for (int dim = 0; dim < Parameter.Dimension; ++dim)
                        {
                            // stepSize[dim] = Probability.logsig((0.5 * Parameter.Iteration - ite) / 20.0) * rand.NextDouble();
                            stepSize[dim] = 0.15 * rand.NextDouble() * Math.Pow(10.0, Math.Exp((-1.0 * ite) / (Parameter.Iteration - ite)));
                            ...
                            {
                                tempSolution[dim] = Parameter.Upper - 0.01 * rand.NextDouble();
                            }
                            else if (tempSolution[dim] < Parameter.Lower)
                            {
                                tempSolution[dim] = Parameter.Lower + 0.01 * rand.NextDouble();
                            }
                        }
                    ...
                    for (int cidx = 0; cidx < Parameter.cluster; ++cidx)
                    {
                        for (int dim = 0; dim < Parameter.Dimension; ++dim)
                        {
                            population[best[cidx], dim] = centersCopy[cidx, dim];
                            fitnessPopu[best[cidx]] = clusterFitnessValues[cidx];
                        }
                    }
                    #endregion
